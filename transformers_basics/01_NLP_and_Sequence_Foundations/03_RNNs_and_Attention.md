# The Evolution to Attention: From ANNs to Bahdanau

Before diving into the Transformer Architecture, it is critical to understand *why* it was invented. The Transformer was not created in a vacuum; it was the final answer to a specific set of architectural bottlenecks that plagued sequence modeling for decades. 

This document traces that exact lineage in six stages: from static neural networks to recurrent loops, the bottlenecks those loops created, and finally, the birth of Attention.

---

## §1 — The Artificial Neural Network (ANN) Limitation

Standard Feed-Forward Artificial Neural Networks (ANNs) have a **hard architectural ceiling on input size**. 

![ANN Fixed Input Structure](../images/ann_fixed_input_structure.svg)

If you design an ANN to accept 5 inputs (e.g., a fixed window of 5 words), the architecture physically hardcodes the number of columns in its weight matrices. It cannot process a 6-word sentence without crashing or requiring you to pad/truncate your data. ANNs have no concept of a "time dimension" or a flexible sequence length.

---

## §2 — Recurrent Neural Networks (RNN): The "Rolled" View

To handle sequences of arbitrary length (like sentences), engineers invented the Recurrent Neural Network (RNN). 

![Single Encoder Cell Reused](../images/single_encoder_cell_reused_v2.svg)

Instead of building a massive network with a hardcoded number of inputs, an RNN is just a **single, reusable cell**. This cell loops over the input data one token at a time. There is no longer a ceiling on input length—the only ceiling is how many calls you are willing to make.

> [!NOTE]
> **Clarification: Encoding $n$ tokens vs. Predicting $n-1$ words**
> 
> A common point of confusion is how many times this cell actually runs. 
> * **Encoding ($n$ calls):** For a sentence of $n$ tokens, the cell physically runs **$n$ times** (one call per token). "The" $\to$ call 1 $\to$ $h_1$; "cat" $\to$ call 2 $\to$ $h_2$; "sat" $\to$ call 3 $\to$ $h_3$.
> * **Language Modeling ($n-1$ predictions):** If you are training this RNN to do next-word prediction, you only get **$n-1$ valid (input, target) pairs**. Why? Because the very last token in your sentence has no "next word" to predict within that sequence!
> 
> Both numbers are real and accurate; they are just answering two different questions (How many states did we compute? vs. How many valid training pairs did we extract?).

---

## §3 — The "Unrolled" View

When reading literature on RNNs, you will almost always see them drawn "unrolled".

![RNN Input to Output Full](../images/rnn_input_to_output_full_v2.svg)

It is crucial to understand that **this is the exact same cell from §2**. It is simply drawn stretched out across time for legibility so we can visualize the flow of the hidden state ($h$) passing from one time step to the next.

---

## §4 — The Tradeoff: Independent Rows vs. Dependent Timesteps

While the RNN solved the variable-length problem, it introduced a massive hardware inefficiency. 

![Independent Rows vs Dependent Timesteps](../images/independent_rows_vs_dependent_timesteps_v2.svg)

* **Independent Rows (ANNs):** The one real freedom that standard rows (or standard batch dimensions) have is *independence*. Because Row 2 does not rely on Row 1, you can batch them and compute them simultaneously on parallel GPU cores.
* **Dependent Timesteps (RNNs):** Timesteps force a **strict sequential order**. You cannot compute $h_3$ until the cell has finished computing $h_2$. This temporal dependency means RNNs cannot easily leverage parallel GPU compute; they must process one word at a time, making them notoriously slow to train.

---

## §5 — Sequence-to-Sequence (Seq2Seq): The Bottleneck

If we want to translate English to French, we use an **Encoder-Decoder** architecture. We use one RNN to read the English (Encoder), and another RNN to write the French (Decoder). 

![No Attention Full Pipeline](../images/no_attention_full_pipeline_v2.svg)

But look at what a standard (no-attention) Encoder-Decoder does with all those hidden states it painstakingly computed: **it throws all but the last one away.**

The entire meaning of the English sentence is squeezed into the final hidden state (the "Context Vector" or bottleneck). If the sentence is 50 words long, the earliest words are heavily compressed or completely forgotten by the time the network reaches the end. 

---

## §6 — Bahdanau Attention: The Final Fix

In 2014, Dzmitry Bahdanau proposed a brilliant fix to the Seq2Seq bottleneck. 

![Bahdanau Decoder Unrolled](../images/bahdanau_decoder_unrolled_v2.svg)

Instead of relying on a single final context vector, Bahdanau kept the same encoder and the exact same hidden states ($h_1, h_2, h_3, \dots$). But now, **every single decoder step reaches back and dynamically re-weights all of the encoder states freshly.**

When translating the word "chat", the decoder learns to assign a high "attention weight" to the encoder state for "cat", and a low weight to everything else. 

This breakthrough bypassed the bottleneck entirely. It is the exact conceptual seed that later evolved into the full "Self-Attention" mechanism at the heart of the Transformer.


---

## §7 — Deep Dive: The Math of the RNN Era and Why It Ended
