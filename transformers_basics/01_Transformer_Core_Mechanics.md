...
**Idea:** Instead of adding position to embeddings, **rotate** Q and K vectors by an angle proportional to their position before computing the dot product.

### Rotation matrix

$$R_{\theta,m} = \begin{pmatrix} \cos(m\theta) & -\sin(m\theta) \\ \sin(m\theta) & \cos(m\theta) \end{pmatrix}$$

- Query at position $m$: $\tilde{q}_m = q_m \cdot R_{\theta,m}^\top$
- Key at position $n$: $\tilde{k}_n = k_n \cdot R_{\theta,n}^\top$

### The key property

$$\tilde{q}_m \cdot \tilde{k}_n^\top = q_m \cdot R_{\theta,m}^\top R_{\theta,n} \cdot k_n^\top = q_m \cdot R_{\theta,n-m} \cdot k_n^\top$$

The dot product depends only on **relative position** $(n-m)$ — position is encoded implicitly in the rotation, without ever being added to the token embedding. RoPE is applied inside the attention layer, not at the input.
